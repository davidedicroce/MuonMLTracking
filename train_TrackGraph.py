...
# EOS/FUSE is read-only for this training job; disabling HDF5 file locks avoids
# spurious lock/stat failures on shared filesystems. Override in the environment
# before launching if you need locking semantics.
...
class H5DatasetIndexError(RuntimeError):
    pass


_H5_RETRYABLE_PATTERNS = (
    "bad symbol table node signature",
    "can't retrieve stat info",
    "cannot retrieve stat info",
...
def _is_retryable_h5_error(exc: BaseException) -> bool:
    msg = str(exc).lower()
    return any(p in msg for p in _H5_RETRYABLE_PATTERNS)


def _sleep_h5_retry(attempt: int, base_sleep_s: float) -> None:
    # Small jitter prevents all DDP ranks/workers from retrying EOS at once.
    delay = float(base_sleep_s) * (2 ** min(int(attempt), 8))
    delay *= random.uniform(0.75, 1.25)
    time.sleep(delay)


def _open_h5_file_with_retries(
    path: str,
    *,
    retries: int = 8,
    ...
    for attempt in range(max(1, int(retries))):
        try:
            ...
            last_err = e
            if not _is_retryable_h5_error(e) or attempt + 1 >= int(retries):
                break
            _sleep_h5_retry(attempt, base_sleep_s)
    raise OSError(
        f"Cannot open H5 file after {int(retries)} attempt(s): {path}; "
        f"last error: {type(last_err).__name__}: {last_err}"
    ) from last_err


def _list_h5_graph_keys_with_retries(
    path: str,
    *,
    retries: int = 5,
    base_sleep_s: float = 0.5,
    ...
    for attempt in range(max(1, int(retries))):
        try:
            with _open_h5_file_with_retries(path, retries=1, base_sleep_s=base_sleep_s, swmr=swmr) as f:
                if "graphs" not in f:
                    return []
    ...
    raise H5DatasetIndexError(
        f"[h5-index] Cannot index H5 file: {path}\n"
        f"           Failed while reading '/graphs' keys.\n"
        f"           Last error: {type(last_err).__name__}: {last_err}\n"
        f"           This usually means the file is corrupt/truncated or EOS/NFS "
        f"returned inconsistent HDF5 metadata. Re-copy/regenerate this file, or "
        f"remove it from --data-glob and regenerate the split file."
    ) from last_err
...
class H5TrackGraphDataset(Dataset):
    ...
    def __init__(
        ...
        h5_paths,
        *,
        h5_index_retries: int = 5,
        h5_index_retry_sleep: float = 0.5,
        h5_open_retries: int = 8,
        h5_open_retry_sleep: float = 0.5,
        h5_read_retries: int = 5,
        h5_read_retry_sleep: float = 0.2,
        h5_max_open_files_per_worker: int = 8,
        h5_close_after_read: bool = False,
        h5_swmr: bool = False,
        bad_graphs_file: Optional[str] = None,
    ):
        self.h5_paths = list(h5_paths)
        if not self.h5_paths:
            raise ValueError("No H5 files provided.")

        self.h5_open_retries = int(h5_open_retries)
        self.h5_open_retry_sleep = float(h5_open_retry_sleep)
        self.h5_read_retries = int(h5_read_retries)
        self.h5_read_retry_sleep = float(h5_read_retry_sleep)
        self.h5_max_open_files_per_worker = int(h5_max_open_files_per_worker)
        self.h5_close_after_read = bool(h5_close_after_read)
        self.h5_swmr = bool(h5_swmr)

        self.index = []
        for fi, p in enumerate(self.h5_paths):
            try:
                keys = _list_h5_graph_keys_with_retries(
                    p,
                    retries=h5_index_retries,
                    base_sleep_s=h5_index_retry_sleep,
                    swmr=self.h5_swmr,
                )
            except Exception as e:
                if bad_graphs_file:
        ...
        # Per-process lazy LRU cache. Do not open all H5 files in every worker:
        # with n_jobs * DDP ranks * num_workers this causes huge EOS metadata bursts.
        self._files: Optional[OrderedDict[int, h5py.File]] = None
        self._pid = None
    ...
    def _open_file(self, fi: int) -> h5py.File:
        return _open_h5_file_with_retries(
            self.h5_paths[fi],
            retries=self.h5_open_retries,
            base_sleep_s=self.h5_open_retry_sleep,
            swmr=self.h5_swmr,
        )

    def _get_file(self, fi: int) -> h5py.File:
        self._ensure_cache()
        assert self._files is not None

        if fi in self._files:
            f = self._files.pop(fi)
            self._files[fi] = f
            return f

        f = self._open_file(fi)
        self._files[fi] = f

        max_open = max(1, int(self.h5_max_open_files_per_worker))
        while len(self._files) > max_open:
    ...
    def _drop_file(self, fi: int) -> None:
        if self._files is None:
            return
        f = self._files.pop(fi, None)
        if f is not None:
            try:
                f.close()
            except Exception:
    ...
    def _read_graph_once(self, fi: int, k: str) -> Dict[str, Any]:
        if self.h5_close_after_read:
            with self._open_file(fi) as f:
                return self._materialize_graph(f, fi, k)
        return self._materialize_graph(self._get_file(fi), fi, k)

    def _materialize_graph(self, f: h5py.File, fi: int, k: str) -> Dict[str, Any]:
        g = f["graphs"][k]
    ...
    def __getitem__(self, idx):
        ...
        for attempt in range(max(1, int(self.h5_read_retries))):
            try:
                return self._read_graph_once(fi, k)
            except Exception as e:
                last_err = e
                # A cached HDF5 handle can become stale on EOS/FUSE. Drop it and retry
                # instead of killing the DataLoader worker immediately.
                self._drop_file(fi)
                if not _is_retryable_h5_error(e) or attempt + 1 >= int(self.h5_read_retries):
                    break
                _sleep_h5_retry(attempt, self.h5_read_retry_sleep)

        raise RuntimeError(
            f"Cannot read graph after {int(self.h5_read_retries)} attempt(s): "
            f"{self.h5_paths[fi]} :: graphs/{k}; "
            f"last error: {type(last_err).__name__}: {last_err}"
        ) from last_err
...
def h5_worker_init_fn(worker_id: int) -> None:
    ...
    try:
        jitter_s = float(os.environ.get("TRACKGRAPH_H5_WORKER_OPEN_JITTER_S", "0") or 0.0)
    except Exception:
        jitter_s = 0.0
    if jitter_s <= 0:
        return
    ...
    rng = random.Random(os.getpid() ^ ((worker_id + 1) * 0x9E3779B1) ^ (rank * 1009) ^ (local_rank * 9176))
...
def main():
    ...
    ap.add_argument("--h5-index-retries", type=int, default=5)
    ap.add_argument("--h5-index-retry-sleep", type=float, default=0.5)
    ...
    ap.add_argument("--worker-open-jitter-s", type=float, default=0.0)
    ...
    os.environ["TRACKGRAPH_H5_WORKER_OPEN_JITTER_S"] = str(float(args.worker_open_jitter_s))
    ...
    if ddp_is_main():
        ...
        print(
            f"[h5] HDF5_USE_FILE_LOCKING={os.environ.get('HDF5_USE_FILE_LOCKING')} "
            f"open_retries={args.h5_open_retries} read_retries={args.h5_read_retries} "
            f"max_open_files_per_worker={args.h5_max_open_files_per_worker} "
            f"close_after_read={args.h5_close_after_read} swmr={args.h5_swmr} "
            f"worker_open_jitter_s={args.worker_open_jitter_s}",
            flush=True,
        )
    ...
    with timed_section("dataset_index", device=index_device, enabled=args.time) as tt:
        ds = H5TrackGraphDataset(
            ...
            h5_open_retries=args.h5_open_retries,
            h5_open_retry_sleep=args.h5_open_retry_sleep,
            h5_read_retries=args.h5_read_retries,
            h5_read_retry_sleep=args.h5_read_retry_sleep,
            h5_max_open_files_per_worker=args.h5_max_open_files_per_worker,
